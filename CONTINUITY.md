...
- The first consumer-app workflow has been moved onto that same rich-client lane too:
  ...
  - The macOS `computer-use` contract review has now been folded into the plugin locally:
    ...
    - `scripts/live_app_server_tidal_image_ab.py` runs `jpeg-q85`, `webp-q80`, `webp-q85`, and `webp-q95` TIDAL variants and writes `artifacts/codex-e2e/tidal-image-ab-summary.json`
...
Use the new rich-client harness as the installed-plugin acceptance path: `scripts/live_app_server_smoke.py` is live-proven for JPEG and WebP, and `scripts/live_app_server_tidal_playlist.py` now has passing full-flow proofs at both `1920x1080` and `1440x810`. The next real seam is no longer TIDAL visibility, “one useless fallback box”, missing image support, generic fallback-only interaction being hopeless, fast-tier setup, compact state, screenshot-pixel coordinate mapping, whether the workflow can complete, whether smaller model-facing screenshots help, whether `1440x900` should be the default, or whether WebP can round-trip through the packaged plugin. The next meaningful move is to run the TIDAL image-format A/B (`scripts/live_app_server_tidal_image_ab.py`) after resetting the playlist, then decide whether WebP beats JPEG on bytes/tokens without hurting task success. Optional floor-finding at `1280x720` can follow if `1440x900` remains the default floor. The full investigation write-up lives in `docs/codex-plugin-e2e-expedition.md`, and the image-size decision is documented in `docs/image-size-performance.md`.
