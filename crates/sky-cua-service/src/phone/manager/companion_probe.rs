...
use sky_cua_platform::model::{
    DiagnosticEntry, PhoneActionResponse, PhoneBackendKind, PhoneCapabilityRefreshState,
    PhoneCompanionCapabilities, PhoneCompanionStatusResponse, PhoneInstallCompanionRequest,
    PhoneSessionSelector, PhoneSettingsScreen,
};
...
use crate::phone::adb::{
    ...
    ensure_secure_list_service,
};
use crate::phone::protocol;
use crate::phone::protocol::identity::{
    CompanionBootstrapOptions, CompanionInstallDecision, InstalledCompanion,
};

impl PhoneManager {
    pub(crate) async fn ensure_companion_permissions(
        &self,
        serial: &str,
        package: &str,
    ...
    pub(crate) async fn flag_companion_permission_gaps(
        &self,
        serial: &str,
    ...
    pub(crate) async fn read_installed_companion(
        &self,
        serial: &str,
        package: &str,
    ...
    pub(crate) fn adb_program(&self) -> String {
    ...
    pub(crate) async fn install_companion(
        &mut self,
    ...
    pub(crate) fn companion_bootstrap_diagnostics(&self, session_id: &str) -> Vec<DiagnosticEntry> {
        self.sessions
            .get(session_id)
    ...
    pub(crate) fn app_action_to_action_response(
        &self,
        selector: &PhoneSessionSelector,
        action: &str,
    ) -> PhoneActionResponse {
...
/// message in `details`.
pub(crate) fn install_outcome_diagnostic(
...
pub(crate) fn permission_outcome_diagnostic(
    label: &str,
...
pub(crate) fn parse_installed_companion(dump: &str) -> InstalledCompanion {
    InstalledCompanion {
...
pub(crate) fn parse_version_code(dump: &str) -> Option<u64> {
...
pub(crate) fn parse_version_name(dump: &str) -> Option<String> {
    for line in dump.lines() {
...
pub(crate) fn parse_signing_cert_sha256(dump: &str) -> Option<String> {
    for line in dump.lines() {
...
pub(crate) fn extract_hex64(line: &str) -> Option<String> {
...
";
    ...
    #[test]
    pub(crate) fn parses_version_code_and_name() {
    ...
    #[test]
    pub(crate) fn parses_signing_cert_sha256_stripping_separators() {
        let installed = parse_installed_companion(DUMP_WITH_CERT_AND_VERSION);
        assert_eq!(
    ...
    #[test]
    pub(crate) fn missing_fields_stay_none_conservatively() {
    ...
    #[test]
    pub(crate) fn ignores_non_digest_hex_in_unrelated_lines() {
