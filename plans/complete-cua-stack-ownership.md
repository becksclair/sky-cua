...
- [ ] (2026-07-20 06:49Z, partial) WP-02: Migrated the complete first-party `cua_node` source/runtime contract and locked assets from the preserved Codex Desktop input into `runtime/cua-node/**`, committed as `530deb5`. Source verification passes 191 tests with 7 intentional production skips. The offline assembler rebuilds first-party outputs under one publication lock, binds the exact 323,846,851-byte/3,579-file migration seed (`b6677535a94b231fcf681ee30d2d1d42ef8028095d45e4040c7eefacda6c32d2`), inventories every shipped package root, emits package/data/native license evidence and SBOM associations, audits native binaries, rejects wrong-platform packages, and marks dirty-source output release-ineligible. Runtime lock and compliance truth now cover pixelmatch 7.1.0, exact Canvas notices, PDF.js CMaps/fonts, deterministic source rebuilds, and generated CycloneDX/SPDX evidence. Development component `9fa976780db0c27f1df128e18f8df1007960608dbc0dc6b4dcca1bcd817336e5` passed exact rebuild/check, static verification, 12-cell persistent REPL media acceptance, and full offline Canvas/Sharp/PDF.js/Tesseract/pixelmatch/Playwright acceptance before the final compliance hardening. Completion requires a clean commit-bound rebuild and repeat of those installed-artifact gates.
- [x] (2026-07-20 07:12Z) WP-03: Implemented canonical `@heliasar/browser-use` with all 72 documented commands supported: 70 decompose onto existing raw/CDP/notification primitives and the final two use explicit daemon-local raw seams. The package preserves exact generated declarations/API fixtures, deterministic byte-identical projections, real Chrome DOM locator execution, WebP annotation, local content/assets, notification-driven events/dialogs/logs/file chooser/downloads, and wrong-hash rejection before connect. Frozen-scope review then found and drove regression-backed fixes for per-tab marks, capability reachability, pre-armed navigation waits, cross-tab download ownership, load-state semantics, modifier chords, CDP clip scale, and asset collisions. `bun run verify` passes 24/24 with zero unsupported commands; canonical `browser-client.mjs` SHA-256 is `622eedccdd1c5904ded78371e2327ebc6a89c4134246514c35093852c250ad66`. The initial complete Browser implementation is committed as `742697f`; the review fixes require one follow-up semantic commit. Installed live Browser acceptance remains WP-09.
...
- [ ] (2026-07-20 07:12Z, partial) WP-05: Added the complete-release producer that sanitizes copied legacy `openai-bundled` Browser and `resources/node_repl` paths from core, materializes both Codex projections from exact canonical Browser bytes, snapshots cua_node under its assembly lock, verifies the inner component plus producer-commit Git source inventory, rejects empty/self-asserted attestations and mixed Browser generations, composes compliance/locks/artifacts, and rejects development-prepared cua_node attestations. The focused release re-review proved two earlier fixes incomplete: cache-only seeds could self-authenticate and core packaging could consume ignored/stale or symlinked worktree inputs. The repaired path independently pins cache pointer, marker, digest, size, and file count before composition/native inspection; builds core into a unique empty directory from validated Git-archive bytes; excludes optional Android, legacy Browser, worktree overlays, and preexisting runtime fallbacks; rejects non-regular archive entries and unattested external inputs; and binds `CORE_BUILD_INPUTS.json` into the release. The producer base is committed as `103c1b1`; 143 focused release/package tests, Ruff, and basedpyright pass. A follow-up semantic fix commit, clean `release_eligible=true` rebuild, installed parity, and unstubbed release creation remain.
...
- [ ] (2026-07-20 06:49Z, partial) WP-07: Ported exact runtime/package/native/data-asset locks, notices, provenance, CycloneDX/SPDX, and recorded the hash-bound current Codex migration-input baseline (`65c69a3`, runtime SHA-256 `eed2bb02daf9ed79add6e96b2b759897e04bde4b393d3049b7fdde4cb93e6b95`). Source runtime verification passes 191 tests with 7 production skips. The installed-candidate transcript runner exact-compares all three MCP tools and the full metadata/persistence/cancellation/module/native/file/image matrix. Browser live acceptance now owns a hermetic loopback fixture, deletes `CODEX_BROWSER_PROVIDER`, and proves tampered/missing/wrong-manifest Browser rejection on disposable packaged copies before connection. `@heliasar/sky-cua` exposes additive v1 `activate_window` context/capability and a packaged full-action runner; its typecheck/build and 47 tests pass, with the window-action contract committed as `83dd279`. Remaining work is the clean candidate's real file/media/Browser/Computer/live-host runs plus candidate benchmark and direct-no-Node-hop topology proof.
- [ ] WP-08: After WP-05 passes, create and monitor exactly one Codex Desktop consumer task limited to resolver precedence, environment hydration, packaged fallback, compatibility materialization, installed acceptance, and producer-code removal.
...
Wave 1 has converged into six ownership-aligned semantic commits: provenance (`c06a76b`), canonical Browser plus final host seams (`742697f`), Computer JS window actions (`83dd279`), first-party node_repl runtime (`530deb5`), immutable release production (`103c1b1`), and two-server host installation (`16b566a`). Source and aggregate gates are green, but no production outcome is claimed until the same clean commit produces an immutable component and release that pass installed-artifact verification. Update this section again at candidate-release acceptance, Codex consumer handoff, installed cutover, and final retirement.
