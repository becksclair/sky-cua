#!/usr/bin/env python3
"""Operator-run KWrite smoke for heuristics-backed editor replacement.

This launches KWrite on XWayland, targets the live editor through MCP, proves
...
KWRITE_TITLE = "proof.txt"
EXPECTED_TEXT = "hello from sky-cua kwrite proof\nsecond line\n"
...
def launch_kwrite(path: Path) -> subprocess.Popen[str]:
    env = dict(os.environ)
...
def find_kwrite_editor(snapshot: dict[str, Any]) -> dict[str, Any]:
    candidates = [
        element
    ...
    if not candidates:
        raise RuntimeError(
            "did not find a plausible KWrite editor element.\n"
...
def main() -> int:
    print("Starting live KWrite smoke.")
    print("If KDE shows a portal approval prompt, approve it so the test can continue.\n")

    require_installed("kwrite")

    with tempfile.TemporaryDirectory(prefix="sky-cua-kwrite-smoke-") as tmpdir:
        tmpdir_path = Path(tmpdir)
        file_path = tmpdir_path / KWRITE_TITLE
        ...
        try:
            ...
            if result.get("isError"):
                ...
                if code == "PortalApprovalPending":
                    raise RuntimeError(
                        "KWrite smoke is still waiting on portal approval. "
                        "Approve the KDE dialog and re-run.\n"
                        f"result={json.dumps(result, indent=2, sort_keys=True)}"
                    )
                raise RuntimeError(
                    "KWrite smoke failed before a snapshot was produced.\n"
                    f"result={json.dumps(result, indent=2, sort_keys=True)}"
                )

            snapshot = result["structuredContent"]
            require_no_portal_approval_pending(snapshot, "KWrite snapshot")
            editor = find_kwrite_editor(snapshot)

            focused_app = snapshot.get("focused_app") or {}
            if (focused_app.get("desktop_file_id") or "") != "kwrite.desktop":
                raise RuntimeError(
                    "KWrite smoke did not focus the expected app.\n"
                    f"focused_app={json.dumps(focused_app, indent=2, sort_keys=True)}"
                )
            if KWRITE_TITLE.lower() not in ((focused_app.get("window_title") or "").lower()):
                raise RuntimeError(
                    "KWrite smoke did not focus the expected file window.\n"
                    f"focused_app={json.dumps(focused_app, indent=2, sort_keys=True)}"
                )
            ...
            require_ok(set_value, "KWrite set_value")
            ...
            if heuristic_diag is None:
                raise RuntimeError(
                    "KWrite set_value did not surface the heuristics-backed fallback diagnostic.\n"
            ...
            if "routing=prefer_physical_fallback" not in details:
                raise RuntimeError(
                    "KWrite set_value did not report the expected routing preference.\n"
            ...
            if final_text != EXPECTED_TEXT:
                raise RuntimeError(
                    "KWrite smoke saved the wrong file contents.\n"
            ...
            print("\nLive KWrite smoke completed successfully.")
            return 0
        finally:
            client.close()
            terminate_process(kwrite, name="KWrite")
