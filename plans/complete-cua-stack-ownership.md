...
- [ ] (2026-07-20 11:01Z, candidate rebuild active) WP-05: Built and hash-verified diagnostic candidate `2529ee922462c73d8ac26d1776bb067c82a2aecbfdc59e62815762774e74fc86` from producer commit `42dd29e5834c88f04cf0df2482439089950a8e7f`; manifest SHA-256 is `1ab27d30b4503d7e8c3066591ddde26aac9523b7c4601498c55b1c94b264ed15`. Its runtime, transaction, tamper, parity, and performance evidence remains valid, but the candidate is rejected for promotion. Commit `fa31fac` adds the checkout-free verified controller as a full/core component, transactional Chrome/Chromium/Brave/Brave-Origin native manifests, exact extension id/version/path/manifest/tree binding, extracted-neutral real install coverage, sanitized buildstamps, checkout-path rejection, and release-wide SBOM/license/provenance coverage. Full Python passes 872/872, focused post-review gates pass, Ruff/basedpyright are green, and independent frozen-diff review found no remaining source defect. WP-05 closes only after a superseding clean-commit immutable candidate builds and the extracted archive configures both hosts and the live exact Browser bridge without checkout/PYTHONPATH dependency.
- [ ] (2026-07-20 10:11Z, partial) WP-06: Implemented verified-generation OpenClaw and OpenCode two-server adapters, committed as `16b566a`. OpenClaw transactionally snapshots/sets/verifies/restores only `sky_cua` and `node_repl`, preserves unrelated servers, reports honest Gateway watcher/restart state, and never treats process-local `mcp reload` as Gateway proof. OpenCode performs targeted JSONC-preserving edits, detects higher-precedence config hazards, creates durable content-addressed backups, supports stale-safe rollback, and requires a full process restart. Both pin one exact generation, trust set, bundled Node/module/Playwright paths, browser socket, and host provenance; focused host/controller tests pass 26/26 and the full Python suite passes 847/847. A real-CLI isolated preflight installed the exact candidate with OpenClaw 2026.7.1 and OpenCode 1.18.3: OpenClaw listed precisely the two bound servers and environments, while a fresh neutral-directory OpenCode process connected both exact-generation `sky_cua` and `node_repl` servers alongside preserved unrelated MCPs. An ordinary OpenCode model then made one code-mode call containing actual `tools.node_repl.js` and `tools.sky_cua.doctor` invocations, observed `{"marker":"opencode-node-repl-ok","value":42}`, and reported the structured candidate daemon backends/readiness. Standalone promotion, real user-config reload/restart, OpenClaw model invocation, and broader installed model workflows remain live gates.
...
- [ ] (2026-07-20 10:24Z, active) WP-08: Created the single gated Codex Desktop consumer task `019f7e76-256d-7973-bd87-4561f0b5b173` from its preserved dirty working-tree state, limited to resolver precedence, environment hydration, packaged fallback, compatibility materialization, installed acceptance, and producer-code removal. It used the rejected candidate for structural resolver/fallback verification only and has been told not to finalize packaging, installed acceptance, or semantic commits against it. Monitor the same task; deliver the superseding release id, manifest hash, root, Browser hash, producer commit, and cua_node tree hash before accepting installed proof and semantic commits.
...
- Observation: the mandatory Brave Origin installation uses a distinct user native-messaging directory, and native-host manifests alone do not prove an exact immutable extension generation.
  Evidence: `/usr/bin/brave-origin` reads `~/.config/BraveSoftware/Brave-Origin/NativeMessagingHosts`, whose live manifest still targets the legacy host; the active separately installed extension is `1.2.27221.15725`, while the release contains three bundled extension versions and previously selected none in `RELEASE.json`.
  Impact: the standalone installer now targets Chrome, Chromium, Brave Browser, and Brave Origin; the producer manifest binds one extension id/version/path/manifest hash/tree hash; installation reports the exact unpacked extension path and a required browser reload. Live acceptance must load that generation path and prove the resulting native-host process path/hash before Browser success counts.
...
Candidate `2529ee922462c73d8ac26d1776bb067c82a2aecbfdc59e62815762774e74fc86` / manifest `1ab27d30b4503d7e8c3066591ddde26aac9523b7c4601498c55b1c94b264ed15` is rejected for promotion despite its valid runtime/parity/benchmark evidence because its archive is not self-installing and its Browser/compliance contracts are incomplete. The single Codex consumer task remains active for structural work but is gated on a superseding exact release. There is no production outcome yet.
