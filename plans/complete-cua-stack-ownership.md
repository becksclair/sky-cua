...
- [x] (2026-07-20 13:31Z) WP-05: Produced and verified final immutable release `f82b61b4962f318b5121464223ba5911d1f66adfed9511ecc42f909fa8b67c11` from producer commit `e2d39d24801d8df96c15aee21914dfe1c7897a57`; `RELEASE.json` SHA-256 is `16fd1878175a6031e0ebc19d77074e1224cc709cf5e83ba7e6b4363315dbd83f`. All seven component trees verify, including the canonical documentation component and pointer-only Codex projection. The cua_node component contains 3,804 files / 324,960,927 bytes with tree SHA-256 `85366b2f2e049fb05d990266d64edbdd7c8e73fcfc64612559ec84e0583c4294`; bundled Node reports 24.14.0 and canonical Browser SHA-256 remains `085ba347a047473272cafc9f024b59c35dca4b29e44dab8b22eaa80e81e7c60d`. Earlier `2a716638…` and `a367d5fe…` generations are superseded evidence, not deployment targets.
- [x] (2026-07-20 11:24Z) WP-06: Installed the exact generation into real OpenClaw and OpenCode configuration with both `sky_cua` and `node_repl`, exact root/trust/module/Node/socket/provenance environment, and hydrated desktop-session environment. OpenClaw gateway restart health verification passed. Fresh ordinary OpenClaw and OpenCode model runs each invoked both servers with zero tool failures and returned Browser ready plus persistent REPL result 42. Broader composed model workflows remain WP-09 acceptance.
- [x] (2026-07-20 10:04Z) WP-07: Ported exact runtime/package/native/data-asset locks, notices, provenance, CycloneDX/SPDX, and recorded the hash-bound current Codex migration-input baseline (`65c69a3`, runtime SHA-256 `eed2bb02daf9ed79add6e96b2b759897e04bde4b393d3049b7fdde4cb93e6b95`). The final clean component passes exact MCP transcripts, persistence/reset/timeout/cancel/meta, native/local-file/buffer/URL/image operations, OCR/PDF/Sharp/Canvas/pixelmatch, WebP/PNG, standalone Playwright, web lifecycle, and all three packaged Browser trust negatives before connection. Its 100-cell benchmark passes: warm median 0.292412 ms (limit 0.52381725), p95 0.57602 ms (limit 1.0988625), and idle RSS 85,905,408 bytes (limit 93,974,528). `@heliasar/sky-cua` typecheck/build and 47 tests pass. Direct topology is proven from the immutable release: `components/core-linux-x64/bin/sky-cua-client mcp` initialized and listed 36 tools, resolves to the native x86-64 client linked only against glibc/libm/libgcc, while `node_repl` is a separate static ELF in the cua-node component. The exact candidate's bundled Node 24 and `node_repl` imported the installed `@heliasar/sky-cua`, preserved persistent bindings across sequential calls, captured/emitted WebP, and performed move/click/drag/scroll/key/type with structured Computer metadata. An isolated exact-candidate daemon (`sky-cua-service` SHA-256 `4f860b2c1af6f86da7fef6af95b2d8ca2658236d6831455fd391d1c923e04f9c`) additionally rejected an ambiguous duplicate KDE title before mutation, accepted its returned KWin UUID, activated the target, and verified focus. Repeating this complete action set after standalone promotion remains a WP-09 installed-process gate alongside IAB/Brave Browser actions.
- [x] (2026-07-20 13:55Z) WP-08: Codex Desktop consumer cutover is committed locally as `1a03a7e1e940f16f6f61b1834a3058f3ba9195c2` (verified adapter), `d97106669f4dac056573ba91778fdcd2abef49fb` (producer retirement), and final closeout `ce15803b` (f82 pin, documentation routing, stale Browser ownership removal, and isolated smoke correction), with no push. Resolver/projection, tamper/fallback, broad source, 20-case final focused, typecheck/lint, Linux tray+IPC smoke, Arch packaging, installed `/opt` verifier, Node/media/OCR/PDF/file/full-subsystem/web-workbench acceptance, and installed-ASAR gates pass. Final package `chatgpt-desktop 26.707.72221-1` SHA-256 is `0e9606abbe8dbbafb56fa5410f9bb4a91775ced0cf3c25044a3915aecd121d14`; ASAR SHA-256 remains `6d5a028ef31c420aa977d79703b07a67f208075dccddaa81e000d54c9ee2720f`. `/opt` pins final release `f82b61b…`, its exact manifest hash, pointer-only documentation routing, and both canonical Browser projections. The upstream plugin identity remains `browser`; `browser-use@openai-bundled` is the generated compatibility/cache identity. The Web Store extension remains installed and untouched.
- [x] (2026-07-20 14:00Z) WP-09: Final release `f82b61b4962f318b5121464223ba5911d1f66adfed9511ecc42f909fa8b67c11` is promoted as standalone `current`; OpenClaw/OpenCode configs and skills, all four native-host manifests, and the active daemon resolve inside that exact generation. The final Codex package is installed under `/opt`; after the single scheduled GUI relaunch, app-server PID `128057` and every Codex `node_repl` child resolved from f82, and no old `/opt/chatgpt-desktop/resources/cua_node` process remained. Installed IAB loaded `browser.documentation()` and opened `https://example.com/`; a separate Brave Origin task with `CODEX_BROWSER_PROVIDER` unset opened and observed the same page through the daemon-owned extension bridge. The Web Store ChatGPT extension remained installed and untouched. Prior passing broad direct/model acceptance was not repeated.
- [x] (2026-07-20 13:02Z) WP-10: The cohesive design is materialized at `docs/research/2026-07-model-facing-cua-docs-phone-js.md`. Canonical compact Browser/Computer/Phone routing skills, shared references/recipes, ten copy-safe examples, deterministic API/capability/example/routing inventories, per-entry hash verification, release compliance/SBOM/provenance binding, pointer-only Codex compatibility routing, exact-generation OpenClaw/OpenCode skill projection, Phone request provenance, and the 27-operation persistent `@heliasar/sky-cua/phone` facade are implemented and committed in `81a1197`, `4e4e74a`, `94c4f64`, and compatibility fix `932a5ec`.
- [ ] (2026-07-20 14:00Z, physical device blocked) WP-11: All ten installed examples pass under bundled Node 24 against final release `f82b61b…`, including awaited image blocks and exact written binary/WebP outputs; the installed Phone singleton exposes terminal state and the generated declaration/API/capability inventories verify. Codex installed Browser/Computer/Phone routing was exercised after restart: IAB loaded canonical Browser documentation, Computer persistent JS captured and emitted a live WebP screenshot, and Phone imported the canonical facade and returned truthful ADB/no-device state. A narrow OpenCode ordinary-model run discovered the f82 Phone skill, followed its persistent `node_repl` route, invoked the facade with `opencode` provenance, and returned truthful no-device state; its final prose incorrectly attributed the package to retained generation `a367d5fe…` after manually searching that old path, so the prose is excluded from generation proof. Exact process/root proof comes from the host configuration and live child paths instead. The corrected focused `wayland-pointer` VM gate passes at `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260720T111641Z`; no broad suite was rerun. A physical Phone is not connected (`adb devices` is empty), so real-device screenshot/action lifecycle parity remains the only external live blocker.
...
- Observation: an immutable release can be internally hash-consistent yet fail the standalone product contract.
  Evidence: candidate `2529ee922462c73d8ac26d1776bb067c82a2aecbfdc59e62815762774e74fc86` passes all 5,085 `SHA256SUMS` entries and full/core component verification, but its archive has no checkout-free installer; release-local Browser preflight exits before native-host registration; its SBOM has no Rust core/native-host/extension subjects; and `sky-cua-client.buildstamp.json` contains `/home/bex/projects/sky-cua` while provenance declares no absolute checkout paths.
  Impact: this candidate is diagnostic evidence only and must never be promoted. WP-05 now requires extracted-neutral-directory installation, browser native-host registration, release-wide compliance subjects, and an absolute-path scan before publishing a superseding immutable identity.

- Observation: the browser bridge must preserve the already-installed Web Store ChatGPT extension; release extension bytes are compatibility/provenance artifacts, not an activation target.
  Evidence: Brave Origin runs Web Store extension `hehggadaopoacecdllhhajmbjkdcmajg` version `1.2.27221.15725`; after standalone installation its native host PID resolves to `2a716638…/sky-cua-chrome-host`, and a real Browser action completed navigation, typing, clicking, readback, and WebP capture through that actor.
  Impact: installers own all four native-messaging manifests, but neither cutover nor acceptance loads an unpacked release extension. Exact generation proof is the immutable native-host executable path/hash plus daemon provenance while the Web Store extension remains active.

- Observation: the Arch packaging boundary can change mode-sensitive immutable component trees even when every file byte and path is unchanged.
  Evidence: the first installed Codex package changed producer modes `0664/0775` to `0644/0755`, producing core tree `a68112…` instead of manifest tree `01010e…`; `rsync -rcni` showed byte identity, the Forge ZIP retained exact modes, and the final package tar exposed the normalization. Explicit no-autoextract plus `bsdtar -p` now preserves the producer modes in both the package tar and `/opt`, and the full installed verifier passes.
  Impact: consumer packages must preserve exact release file modes and continue using the strict producer tree verifier; mode drift is never papered over by weakening hash verification.

- Observation: a verifier extension must not retroactively require a newly introduced capability record in retained valid generations.
  Evidence: the first final-phase promotion rejected existing release `2a716638…` because release-wide provenance validation unconditionally required `model-facing-documentation`, even though that generation did not advertise the capability. Capability-gating the provenance/path/license expectation verifies both the retained generation and documentation-bearing candidate; 67 focused release tests pass.
  Impact: new documentation releases remain strict while prior complete generations remain valid inputs to atomic promotion and recovery.
...
- Decision: Keep rollback as a bounded generation/reprojection sanity gate and prioritize successful clean release installation and live host deployment.
  Rationale: the retained prior generation and checkout remain operational recovery inputs; additional rollback machinery must not delay the critical path of neutral install, exact Browser bridge activation, and real Codex/OpenClaw/OpenCode tool execution.
  Date/Author: 2026-07-20, Codex and user steer.

## Outcomes & Retrospective

The ownership migration produced final immutable release `f82b61b4962f318b5121464223ba5911d1f66adfed9511ecc42f909fa8b67c11` / manifest `16fd1878175a6031e0ebc19d77074e1224cc709cf5e83ba7e6b4363315dbd83f`. It is the active standalone, Codex Desktop, OpenClaw, and OpenCode generation. The complete direct and persistent stacks, canonical Browser bytes, Phone facade, installed model documentation, examples, compliance records, performance gates, consumer package, IAB, and Brave Origin are verified on installed artifacts. The Web Store extension remained untouched, and Codex producer ownership was removed in the dedicated consumer commits.

The plan remains active for one truthful reason: no physical Android device is connected, so the final persistent Phone JS device screenshot/action lifecycle cannot be proven. The no-device lifecycle and provenance behavior pass. No broad suite or rollback exercise will be repeated while waiting for that external state; when a device is available, only the narrow Phone acceptance and final plan retirement remain.
...
Outcome: the standalone current generation and all three hosts run the accepted release, with concurrency/provenance proven. Rollback remains only a bounded sanity check and does not delay successful deployment.
