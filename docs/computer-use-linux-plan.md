...
- Decision: treat `codex exec` as a diagnostic harness and `codex app-server` as the installed-plugin acceptance harness.
  ...
  Date/Author: 2026-04-23 / Sarah
- Decision: present the Codex-compatible bundled plugin as `computer-use`, but keep `sky-cua-client mcp` as the runtime boundary.
...
The current milestone now proves much more than the initial architectural slice. This repository contains a real Codex plugin package, a Rust client/service split, a live Linux environment probe, AT-SPI-backed app enumeration, focused-app selection, flattened accessibility snapshots, heuristics resolution, cached portal session ownership, semantic AT-SPI action routing, physical Wayland input routing through the RemoteDesktop portal, conservative X11/XWayland fallback-region hints, operator-facing portal lifecycle diagnostics, an explicit split between the primary capture lane and the actual image-producing lane, a minimal rich-client `codex app-server` harness that proves installed-plugin acceptance with real approval round-trips, and now a Codex Desktop compatibility lane that makes Linux Computer Use appear as `computer-use` while keeping Browser Use and Chrome companions installed.

The snapshot contract now also includes Linux AT-SPI readback for the controls where it matters most. Editable or focused text controls can expose `value` and bounded `text.content`, AT-SPI Value controls can expose `numeric_value`, and sensitive fields preserve metadata while suppressing content. The direct VM smoke proved `zenity` initial text readback, post-`set_value` readback, and post-`type_text` readback; the Codex exec and rich app-server smokes then proved agents can actually consume that information from `get_app_state` before submitting the dialog.
...
Revision note: updated the plan to record the blocked cross-desktop live-smoke matrix and link it to the GUI desktop harness contract instead of treating unit/parser coverage as compositor proof.
