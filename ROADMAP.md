...
- [x] Isolated xpra desktop (private X11 desktop the computer-use agent owns;
  ...
  - [x] Live host-leak and headline `desktop_launch_app` end-to-end run plus the
...
- [x] Complete CUA stack ownership — one fixed-root native + persistent Node
  ...
  - [x] Pinned Acorn and Acorn Walk `node_repl` loaders for generated/minified
        ...
        acceptance
  - [x] Hermes Agent deploy target with transactional two-server config
        projection and real model-turn MCP acceptance —
        [`docs/features/hermes-agent-integration.md`](docs/features/hermes-agent-integration.md)
...
- [x] Consolidated agent test matrix and tool-use performance judge — [`docs/features/codex-cua-tool-use-gate.md`](docs/features/codex-cua-tool-use-gate.md)
  - [x] opencode/pi collapsed from four dialog-dismiss runs to one read-only
        wiring check each on their centralized model profiles
  ...
  - [x] Host-side `performance_judge` model profile scores tool-use 0-100, hard-fails below a
