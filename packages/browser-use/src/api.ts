...
const DOCUMENTS: Record<string, string> = {
  ...
  "browser-troubleshooting": "Re-list browsers and tabs, then report the structured backend error without changing providers implicitly. After js_reset, an extension tab may remain owned by the prior synthetic node_repl session; retry browser.user.claimTab(tab, { reclaimStale: true }) once to finalize only that stale node_repl owner and reclaim the tab.",
  ...
  "tab-claiming-chrome": "Claim an existing user tab explicitly before controlling it. If the tab is owned by a prior synthetic node_repl session after js_reset, retry once with browser.user.claimTab(tab, { reclaimStale: true }); non-node_repl owners are never reclaimed.",
