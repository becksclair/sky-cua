...
- [x] (2026-06-22 14:25Z) Continued the VM matrix across real desktop sessions. The i3/X11 overlay smoke passed in `/workspace/artifacts/codex-e2e/agent-cursor-x11-overlay/20260622T142051272418Z`. GNOME initially failed because the fixture window could not be enumerated; running `sky-cua-service setup-window-targeting` installed and enabled the bundled GNOME Shell extension, the GNOME session restart made the extension DBus API available, and `wayland-pointer` then passed click, secondary click, drag, scroll, and text entry in `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260622T142452Z`.
- [x] (2026-06-22 14:33Z) Proved the VM OpenCode host path against the compact profile. The first VM rerun exposed two launch-path defects: installed agent binaries copied without the `.buildstamp.json` sidecar tripped the live-test freshness gate, and the VM profile did not forward `SKY_CUA_MCP_TOOL_PROFILE=compact` as an explicit installer flag, so persisted legacy state silently won. The installer now carries build stamps with copied executables and removes stale destination stamps when the source is unstamped; the VM runner/profile now forwards explicit MCP launch policy into OpenCode installation. Verified readback showed `/home/skycua/.local/share/sky-cua/opencode.json` and `/workspace/opencode.json` both pinned `SKY_CUA_MCP_TOOL_PROFILE=compact`, and the real OpenCode smokes dismissed Zenity and kdialog with tool evidence in `/workspace/artifacts/opencode-zenity-smoke/20260622T143217Z` and `/workspace/artifacts/opencode-kdialog-smoke/20260622T143242Z`.
- [x] (2026-06-22 14:38Z) Extended the same explicit launch-policy forwarding to the Pi VM profile and proved compact materialization through Pi's wrapper-based MCP launch path. Pi does not carry MCP environment in `mcp.json`; it exports policy from `pi_mcp_wrapper.sh`. VM readback showed `export SKY_CUA_MCP_TOOL_PROFILE=compact`, and a direct stdio `tools/list` through `/home/skycua/.local/share/sky-cua/pi_mcp_wrapper.sh` returned 34 compact tools with `status` and `desktop_action` present and legacy `phone_status`/`click` absent. A live Pi fixture run reached the agent but did not dismiss Zenity in `/workspace/artifacts/pi-zenity-smoke/20260622T143747Z`; it selected the Pi smoke default `openai-codex/gpt-5.5`, so further live Pi agent retries are deferred until an approved non-OpenAI Pi model/auth path is pinned.
- [x] (2026-06-22 14:41Z) Re-ran the KDE host-framebuffer proof after returning the VM to a clean Plasma session. The previous host marker miss did not reproduce: `kde-kwin-effect-system-install` passed with `host_marker_probe.found=true`, 581 changed pixels near the hotspot, KWin effect load/unload proof, visible overlay capture, and cleanup in `/home/bex/projects/sky-cua/artifacts/kde-framebuffer-cursor-proof/kwin-system-install/20260622T144120387631Z`.
- [x] (2026-06-22 14:55Z) Closed the final deploy-freshness gap exposed by the installed bundle probe. `scripts/build_plugin.py` now stamps the freshly built release client and copies `.buildstamp.json` sidecars into staged runtime paths; `scripts/package_runtime_artifact.py` produces those sidecars for per-platform runtime artifacts; `_plugin_bundle.merge_runtime_artifacts` carries the same sidecars for multi-platform artifact assembly. The full verification pass after the fix was `cargo fmt --check && cargo test`, `uv run ruff format --check scripts && uv run ruff check scripts && uv run basedpyright && uv run pytest` with 531 Python tests passing, `python3 scripts/build_plugin.py`, `python3 scripts/deploy_plugin.py --mcp-tool-profile compact --local-install-host opencode`, and `python3 scripts/probe_mcp_tool_surface.py --profile both --installed`, which passed compact 34-tool and legacy 66-tool profile isolation from the deployed bundle.
...
- 2026-06-22: The final ultra-review found compact schema/dispatch parity holes that would have let real agents form invalid calls: `phone_notification_reply` omitted required `action_id`, `phone_app_action` advertised unsupported intent fields instead of required `intent_uri`, `phone_app_install` advertised unsupported install modes and accepted `apk_path` without normalizing it to legacy `apk_paths`, and `desktop_pointer` top-level `required` fields rejected valid drag-only branches. A rerun then found more schema-contract drift: `phone_setup` advertised wrong Android settings screen names and did not require `screen` for `open_settings`, `phone_pointer`/`phone_keyboard`/`phone_notification_action` allowed operation-only calls that the legacy dispatch rejects, and desktop selector tools still required `element_index` even though their schemas exposed alternate selectors. A third rerun found the same parity issue in read/browser branches: browser observe/capture calls could omit legacy-required `tab_id`, `browser_scroll` could omit a scroll delta, and `list_resources` exposed invalid surface/resource pairs. A fourth rerun found `desktop_scroll` still advertised horizontal directions unsupported by the current Linux scroll executor, and that generated call cases were not validating minimal arguments against the emitted schemas. A fifth rerun found browser URL schemes were advertised broader than the parser accepts, `browser_scroll` allowed one-sided x/y coordinates, and `phone_pointer` coordinates allowed negative values. A sixth rerun found `phone_app_install` allowed empty `apk_paths` and omitted `grant_runtime_permissions`, while `desktop_scroll` advertised ignored `amount` instead of the legacy `pages`/`steps`/`delta_y` fields. A seventh rerun found `activate_window` allowed an empty call that legacy dispatch rejects; compact now requires at least one target property and rejects blank string targets plus zero PID targets. An eighth rerun found the call-case validator was not enforcing `additionalProperties: false`, so invalid keyboard examples slipped through; validation now enforces that and compact keyboard examples use the dispatch-supported key/text-only shape with optional window targets. A ninth rerun found browser schemas still allowed empty `tab_id` strings; the shared browser tab id schema now requires non-empty values and fixture validation enforces that. The compact schemas and call adapter now match the legacy handlers, generated contract fixtures cover the repaired branch fields, direct unit assertions pin the risky constraints, generated call cases are schema-validated, and the installed profile probe verifies the rebuilt MCP wrapper exposes the repaired compact surface. Verification after these repairs: `cargo fmt --check`, `cargo test -p sky-cua-client compact_`, `cargo test -p sky-cua-client mcp_tools::definitions::annotation_tests::mcp_tool_surface_matrix_fixture_matches_generated_registry`, `python3 scripts/build_plugin.py`, `python3 scripts/probe_mcp_tool_surface.py --profile both --installed`, `cargo test`, and `uv run ruff format --check scripts && uv run ruff check scripts && uv run basedpyright && uv run pytest`.
- 2026-06-22: A later dedicated ultra-review rerun found compact required strings still accepted empty values for keyboard/browser/notification/app/eval branches; those schemas now share non-empty string constraints for required text/key/id/package/intent/expression fields while keeping optional selector strings small enough to stay inside the 65% serialized budget. The same rerun questioned the COSMIC 2x virtual-input workaround on unscaled displays, but later live VM evidence showed unscaled COSMIC also lands ydotool absolute coordinates at 2x the request, so the production fix keeps the 2x correction for COSMIC Wayland. Verification: compact schema tests, fixture regeneration and fixture/call-case validation, `cargo fmt --check && cargo test`, `uv run ruff format --check scripts && uv run ruff check scripts && uv run basedpyright && uv run pytest`, `python3 scripts/build_plugin.py`, and `python3 scripts/probe_mcp_tool_surface.py --profile both --installed`.
- 2026-06-22: The next ultra-review rerun found three more schema-policy mismatches: desktop selector branches allowed snapshot-bound selectors without `snapshot_id`, empty desktop selector strings still satisfied compact schemas, and the client accepted broader truthy values for `SKY_CUA_BROWSER_EVAL` than the service execution gate. Compact desktop selector schemas now require `snapshot_id` with `element_index`, `name`, and `text`, while direct `element_identifier` can still stand alone and all selector strings are non-empty. Browser eval advertisement now uses the same `on`/`1`/`true` truthy policy as service execution. Verification: focused compact schema/parser tests, regenerated compact fixtures and call cases, the 65% serialized-size gate, `cargo fmt --check && cargo test`, `uv run ruff format --check scripts && uv run ruff check scripts && uv run basedpyright && uv run pytest`, `python3 scripts/build_plugin.py`, and `python3 scripts/probe_mcp_tool_surface.py --profile both --installed`.
- 2026-06-22: A subsequent ultra-review rerun found that the COSMIC coordinate transform still dropped non-zero display origins, and that compact pointer/scroll schemas still advertised direct `element_identifier` targeting even though those physical action paths cannot honor backend refs. The Linux virtual-input transform now preserves the selected display origin when dividing scaled COSMIC coordinates, with a two-display regression test. Compact pointer and scroll schemas now advertise only explicit coordinates or snapshot-resolved selectors; direct `element_identifier` remains available for semantic element actions where dispatch uses it. Verification: focused Linux origin and compact schema tests plus regenerated fixtures and call-case validation.
- 2026-06-22: The next review rerun found the same unsupported direct backend-ref shape still present for drag start targets. The compact drag schema now accepts explicit coordinate starts or snapshot-resolved start/end element indexes only, with a regression assertion and regenerated fixtures.
- 2026-06-22: Another review rerun found the compact `desktop_action` named branch used the ambiguous legacy `action` alias and questioned the fixed COSMIC 2x coordinate correction. Compact `perform_action` now advertises `action_name` or `action_index`, matching the executor contract directly. Subsequent live COSMIC VM proof showed the correction is a ydotool/COSMIC virtual-device quirk rather than the display scale: unscaled coordinates emitted at `(179,240)` landed at `(358,480)`, so COSMIC Linux virtual input keeps the 2x correction while preserving display origins.
- 2026-06-22: A final ultra-review rerun flagged ydotool absolute mouse movement `-x`/`-y` flags as suspicious, but the real COSMIC VM scaled pointer lane regressed when changed to positional `-- x y`: the fixture saw motion at `(0,0)`. The VM's installed `ydotool mousemove --help` advertises `[-x <xpos> -y <ypos>] [-- <xpos> <ypos>]`, and the proven `-x`/`-y` argv shape is pinned by the unit test and live COSMIC proof.
- 2026-06-22: The following ultra-review rerun found the earlier portal-first Wayland auto policy was too broad: it fixed Plasma pointer-origin failures but bypassed working Linux virtual input on Wayland sessions that should use it. Auto input selection is now desktop-aware: KDE/Plasma and GNOME keep portal-first, COSMIC and other Wayland sessions prefer available Linux virtual input and fall back to RemoteDesktop portal only when virtual input is unavailable or explicitly selected.
- 2026-06-22: Revalidated the narrowed Wayland input policy in the real testing VM. Plasma/KDE `wayland-pointer` on `wayland-0` passed with `input_backend=portal_remote_desktop` and RemoteDesktop EIS actions in `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260622T171053Z`. COSMIC `wayland-pointer-scaled` on `wayland-1` passed with `input_backend=linux_virtual_input` and `LinuxVirtualInputCoordinateScale` diagnostics using `coordinate_scale=2.00` in `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260622T171913Z`.
