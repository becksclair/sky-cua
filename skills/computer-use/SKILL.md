...
If the task is inside a browser page and `browser_*` MCP tools are available, use
the `browser-use` skill instead. Desktop `click`/`drag` coordinates come from
`get_app_state` screenshots; browser actions use `browser_screenshot`
coordinates. Do not mix the two coordinate spaces.
