...
- [x] (2026-07-20 06:49Z) WP-03: Implemented canonical `@heliasar/browser-use` with all 72 documented commands supported: 70 decompose onto existing raw/CDP/notification primitives and the final two use explicit daemon-local raw seams. The package preserves exact generated declarations/API fixtures, deterministic byte-identical projections, real Chrome DOM locator execution, WebP annotation, local content/assets, notification-driven events/dialogs/logs/file chooser/downloads, and wrong-hash rejection before connect. `bun run verify` passes 18/18 with zero unsupported commands; canonical `browser-client.mjs` SHA-256 is `4798f74e3ccd0a767a0c99cd72cf29bc03050192b4a76659a19aae97701d8d9f`. The complete Browser implementation is committed as `742697f`; installed live Browser acceptance remains WP-09.
...
- [ ] (2026-07-20 06:49Z, partial) WP-05: Added the complete-release producer that sanitizes copied legacy `openai-bundled` Browser and `resources/node_repl` paths from core, materializes both Codex projections from exact canonical Browser bytes, snapshots cua_node under its assembly lock, verifies the inner component plus producer-commit Git source inventory, rejects empty/self-asserted attestations and mixed Browser generations, composes compliance/locks/artifacts, and rejects development-prepared cua_node attestations. Independent re-review closed all five initial producer findings; structured autoreview then found and drove fixes for migration-seed authentication, recursive symlink rejection, producer-commit core binding, pre-audit seed trust, and dangling-current recovery. Focused negative tests cover those failures. The producer is committed as `103c1b1`; full Rust nextest passes 1,331/1,331, full Python pytest passes 847/847, Ruff and basedpyright pass, and plugin staging succeeds. The immutable candidate still requires the in-flight focused release re-review, a clean `release_eligible=true` rebuild, installed parity, and unstubbed release creation.
