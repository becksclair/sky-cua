...
- The preferred Linux desktop test harness is now an Arch `testing-vm`, not the retired Docker GUI harness. `scripts/testing-vm/provision-arch-testing-vm.sh` installs the desktop matrix, matching terminal apps, Chrome, OpenCode, optional Codex Desktop package, greetd autologin, SSH, and rsync; `scripts/run_gui_testing_vm_smoke.py` builds sky-cua binaries on the host, syncs the checkout into `/workspace`, optionally syncs selected `~/.codex` state only with `--sync-codex-settings`, and runs profiles over SSH against real guest sessions.
  - current live VM proof: QEMU/libvirt domain `testing-vm` booted into COSMIC Wayland with `cosmic-session`, `cosmic-comp`, and `/run/user/1000/wayland-1`; `cosmic-helper` proved `sky-cua-cosmic-helper` listing/activation/focused-window readback at `/workspace/artifacts/gui-desktop-smoke/cosmic-helper/20260515T031400Z/`
  - COSMIC portal selection requires `XDG_CURRENT_DESKTOP=COSMIC`; with that set, `xdg-desktop-portal` selects `org.freedesktop.impl.portal.desktop.cosmic` and exposes ScreenCast/Screenshot, but current upstream/Arch `xdg-desktop-portal-cosmic` does not advertise RemoteDesktop. Do not force GNOME/KDE RemoteDesktop under COSMIC; use `LinuxVirtualInput` for COSMIC physical pointer smokes when virtual input is available.
  - latest COSMIC VM input run: `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260515T093737Z` proves the repeatable `wayland-pointer-scaled` profile on COSMIC Wayland at 1600x1200 with `Scale: 125%`, with `clicked=true`, `drag_completed=true`, `scroll_events=1`, `entry_text="cosmic-text-smoke"`, and `submitted_text="cosmic-text-smoke"`; the profile restored the display to 1280x800 at 100% scale afterward. The 1x proof is `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260515T092606Z`. The earlier blocker artifact `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260515T034151Z/` failed with `ActionUnsupportedForEnvironment: no physical input backend is available for click fallback` before the backend existed.
  - COSMIC physical input now uses `InputBackendKind::LinuxVirtualInput`: the public backend is wired through platform model, Wayland backend selection, Linux click/secondary-click/scroll/drag/type_text/press_key, and the physical `set_value` fallback. Live calibration rejected ydotool as the pointer adapter because its VM device is relative-only and `mousemove --absolute` landed at accelerated/doubled coordinates; the accepted pointer adapter is direct absolute `/dev/uinput` with bounds and scale from `cosmic-randr`/`xrandr`/environment overrides, converting desktop logical points to physical absolute-device coordinates at the uinput boundary. `ydotool` remains the text/key adapter and now has a COSMIC fixture proof. Latest local proof after the direct-uinput slice includes `cargo fmt --check`, `cargo test -p sky-cua-platform`, focused Linux virtual-input/env-probe tests, and full `cargo test -p sky-cua-linux`.
  - OpenCode is installed live on the VM as `opencode 1.14.51`; `scripts/testing-vm/sync-opencode-to-vm.sh` copies host `~/.config/opencode` plus `~/.local/share/opencode/auth.json` without the multi-GB local OpenCode DB/snapshots, and `opencode models openai` succeeds on the VM.
  - terminal coverage is installed live in the VM: `cosmic-term`, `konsole`, `gnome-terminal`, `kgx`, `foot`, `xterm`, `alacritty`, `kitty`, `wezterm`, `ghostty`, and `i3-sensible-terminal` resolve on PATH.
  - latest Plasma VM cursor proof: the session launcher now uses the normal user DBus bus instead of `dbus-run-session`, with the KDE locker disabled and PowerDevil masked for the test image. KWin DBus discovery works from SSH. The clean service-smoke sequence after stale-portal/probe/lifecycle fixes passed synthetic `/workspace/artifacts/codex-e2e/agent-cursor-kde/0515100302670580-syn`, visible layer-shell `/workspace/artifacts/codex-e2e/agent-cursor-kde/0515100303845615-vis`, hide-for-capture `/workspace/artifacts/codex-e2e/agent-cursor-kde/0515100305142807-hide`, and click-through `/workspace/artifacts/codex-e2e/agent-cursor-kde/0515100306568235-click`, leaving no stale `sky-cua-service`, `sky-cua-overlay-host`, or `cosmic-randr` processes.
  - latest KDE full pointer smoke: `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260515T100113Z` proves click, drag, scroll, `type_text`, and `press_key` through the RemoteDesktop portal on the fullscreen GTK fixture after the GNOME geometry, portal-scroll, stale portal, and COSMIC probe fixes.
  - latest GNOME full pointer smoke: `/workspace/artifacts/gui-desktop-smoke/wayland-pointer/20260515T084643Z` proves click, drag, and scroll through GNOME Wayland's RemoteDesktop portal (`clicked=true`, `drag_completed=true`, `scroll_events=2`). The session selector now stops the inactive display manager on session switches; otherwise GNOME and Plasma can run side by side and poison portal/backend evidence.
  - latest i3/X11 overlay proof: `/workspace/artifacts/codex-e2e/agent-cursor-x11-overlay/20260515T075301057704Z` from `scripts/run_gui_testing_vm_smoke.py --profile i3` proves `x11_shaped_window` visible capture, hide, re-show, click-through, and XFixes system cursor hide/show on a real Xorg/i3 guest session. The i3 profile now reconstructs the real `DISPLAY`/`XAUTHORITY` from Xorg when the user environment still contains stale Wayland or Xwayland values.
  - latest KWin effect production check: user-level compiled-effect discovery remains blocked (`/workspace/artifacts/codex-e2e/agent-cursor-kde/0515075621741796-kwin`), but VM system install plus Plasma restart breaks through discovery and is now automated through the VM runner. `scripts/run_gui_testing_vm_smoke.py --profile kde-kwin-effect-system-install --vm-name testing-vm --libvirt-uri qemu:///session` installs the effect under `/usr`, restarts Plasma, waits for the guest ready file, captures before/after host framebuffers with `virsh`, verifies the cursor at `(420,260)`, and lets the guest hide/uninstall/restart. Latest artifact `artifacts/kde-framebuffer-cursor-proof/kwin-system-install/20260515T100852814643Z/host-summary.json` has `ok=true`, `host_marker_probe.found=true`, `changed_pixels_near_hotspot=186`, `max_channel_delta_near_hotspot=168`, remote `backend=kwin_effect`, remote `system_cursor_hidden=true`, and no cleanup leftovers.
  - latest Hyprland layer-shell overlay proof: `/workspace/artifacts/codex-e2e/agent-cursor-wayland-layer-shell/20260515T080912397166Z` from `scripts/run_gui_testing_vm_smoke.py --profile wayland-layer-shell-overlay --desktop-env Hyprland --wayland-display wayland-1` proves `backend=wayland_layer_shell`, visible cursor capture through `grim -o Virtual-1`, click-through capability, and hide-for-capture in a real Hyprland VM session. The first Hyprland attempt exposed a real portability bug: unconfigured layer-shell output surfaces must not receive buffers. `crates/sky-cua-overlay-host/src/layer_shell.rs` now skips unconfigured layer surfaces before drawing.
  - current compositor cursor-hiding implementation covers KWin, X11/i3, Hyprland, GNOME Shell, and patched COSMIC with real VM proof. Latest proof artifacts from the compositor cursor plan are Hyprland `/workspace/artifacts/codex-e2e/agent-cursor-wayland-layer-shell/20260515T142710878162Z/`, KWin `artifacts/kde-framebuffer-cursor-proof/kwin-system-install/20260515T132649888064Z/host-summary.json`, X11 `/workspace/artifacts/codex-e2e/agent-cursor-x11-overlay/20260515T142731049499Z/`, GNOME `artifacts/gnome-framebuffer-cursor-proof/20260515T140437893805720Z/host-summary.json` from a normal GDM GNOME Wayland user session, and COSMIC `artifacts/cosmic-framebuffer-cursor-proof/20260515T142538562074Z/host-summary.json` from a patched `cosmic-comp` built at Arch's installed source commit `b5a1a6d3179810627fa0bffac7bd5d78c7df4fa0`. Unpatched COSMIC remains honestly unsupported: no `$XDG_RUNTIME_DIR/sky-cua-cosmic-cursor-ready` sentinel means `system_cursor_hide_supported=false`.
  - unpatched controlled-VM COSMIC now has a separate transparent-Xcursor session mode: `cosmic-blank` / `cosmic-transparent` generates `sky-cua-blank`, starts COSMIC with `XCURSOR_THEME=sky-cua-blank`, and uses `system_cursor_backend=cosmic_transparent_xcursor`. Proof: `artifacts/cosmic-transparent-xcursor-cursor-proof/20260516T073232164704Z/host-summary.json` reports `ok=true`; this mode preserves one visible cursor while the agent overlay is visible but does not restore a normal native cursor when the overlay hides.
...
- Embedded-X11 VM smokes are retired. X11 acceptance for the VM must boot or
  select a real X11 session and run the X11 profile against that guest display.
- Snapshotless physical/focused actions now probe the environment on the service side, so `click`, `perform_secondary_action`, `drag`, `scroll`, `type_text`, and `press_key` can run without a cached snapshot when they do not reference an element. Element-index, `set_value`, and semantic actions remain snapshot-scoped because they need the cached accessibility context that produced the element ids.
- The MCP tool schemas now say that coordinate actions with a `snapshot_id` use screenshot pixel coordinates, while coordinate actions without a `snapshot_id` use current screen coordinates for the active input backend.
- The desktop smokes now also prove the structured capture truth:
...
- The release plugin was previously rebuilt and installed through `python3 scripts/deploy_release_plugin.py`; `codex mcp list --json` reported `sky-cua@sky-cua-local` enabled with the installed `computer-use` server, and `scripts/live_app_server_smoke.py` passed against that installed bundle at `artifacts/codex-e2e/app-server-smoke/20260508T112000Z/`. The release marketplace is now migrating to the standalone Heliasar marketplace and `sky-cua@Heliasar`.
...
- The first consumer-app workflow was moved onto that same rich-client lane and is now retired as an active live workflow:
  - the former TIDAL playlist workflow used `codex app-server` instead of `codex exec`
  ...
  - the first full TIDAL rich-client workflow passed live with the stronger workflow settings:
    - the retired harness used `gpt-5.5`, medium reasoning, and a 420-second turn budget
  ...
  - that retired lane once centralized workflow constants, schema path, and result validation so the app-server and exec harnesses did not drift on model, reasoning, playlist name, song count, or validation behavior
  ...
  - the tightened TIDAL workflow passed with the macOS-style screenshot coordinate contract before retirement:
    - live proof: `artifacts/codex-e2e/tidal-playlist-app-server/20260424T035836Z/`
  ...
  - The macOS `computer-use` contract review has now been folded into the plugin locally:
    ...
    - the service now rejects actions without a snapshot as `ComputerUseInactive`, and rejects non-latest snapshots as `SnapshotStale`, matching the observe-act-observe contract more closely
    - `scripts/_app_server_harness.py` now preflights `mcpServerStatus/list` before `thread/start`, writing `mcp-server-status.json` so server/tool visibility fails fast
    - model screenshots can now be encoded as lossy WebP through `SKY_CUA_MODEL_SCREENSHOT_FORMAT=webp` plus `SKY_CUA_MODEL_SCREENSHOT_WEBP_QUALITY`; JPEG remains the default
    - capture metadata now includes `model_image_format`, `model_image_quality`, `model_image_bytes`, and `model_image_encode_ms`
    - the former TIDAL live workflow has been retired; active live-smoke model configuration is centralized in `scripts/_smoke_config.py` as `gpt-5.5` with low reasoning
    - live proof: `scripts/live_app_server_smoke.py` passed at `artifacts/codex-e2e/app-server-smoke/20260424T050517Z/` with stricter `snapshot_id` action flow
    - WebP live proof: `SKY_CUA_MODEL_SCREENSHOT_FORMAT=webp SKY_CUA_MODEL_SCREENSHOT_WEBP_QUALITY=85 python3 scripts/live_app_server_smoke.py` passed at `artifacts/codex-e2e/app-server-smoke/20260424T050553Z/`, returning a `.webp` screenshot path and capture metadata `model_image_format=webp`, `model_image_quality=85`, `model_image_bytes=94664`, `model_image_encode_ms=69`
