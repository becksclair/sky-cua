...
- [ ] (2026-07-20 11:44Z, live acceptance active) WP-09: Promoted `~/.local/share/sky-cua/current` to `2a716638…`, installed/reloaded OpenClaw and OpenCode, and proved the active `sky-cua-service` and Web Store extension native host resolve to immutable release binaries. Installed transcript acceptance passes persistence/top-level-await/reset/timeout/cancel/meta/native/local-file/image cases; installed full subsystem acceptance passes Canvas, Sharp, PDF.js, Tesseract, pixelmatch, WebP, and system-Chrome Playwright. Codex packaged fallback and exact installed component verification now pass. The task-hosting GUI still predates package replacement, so its Browser socket has no listener; one real Codex restart, post-restart IAB/Brave proof, and the final three-host concurrent/composed matrix remain.
- [ ] (2026-07-20 12:34Z, implementation active) WP-10: The cohesive design is materialized at `docs/research/2026-07-model-facing-cua-docs-phone-js.md`. Canonical compact Browser/Computer/Phone routing skills, shared references/recipes, copy-safe examples, deterministic API/capability/example/routing inventories, per-entry hash verification, release compliance/SBOM/provenance binding, pointer-only Codex compatibility routing, and exact-generation OpenClaw/OpenCode documentation-root hydration are implemented. Focused documentation/release/builder/installer tests pass (86 tests in the combined gate). Persistent Phone protocol and JS facade lanes are converging; the final serial GUI restart/live Browser action remains deferred until this new generation is installed so it occurs once.
