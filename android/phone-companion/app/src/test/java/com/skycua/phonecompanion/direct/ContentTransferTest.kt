...
class ContentTransferTest {
    @Test fun goldenSmallFirstChunkMatchesFrozenFixture() {
        val bytes = DirectContentChunkCodec.encode("t1", 1, 0, 0, byteArrayOf(1, 2, 3))
    ...
    @Test fun senderDeclaresChunksInOrderAndCommitsShaAndLength() {
        ...
        val sender = ContentTransferSender(socket, { "device" }, { 4 }, idFactory = sequenceOf("content", "transfer").iterator()::next)
    ...
    @Test fun disconnectClearsActiveTransferAfterSendFailure() {
        ...
        val sender = ContentTransferSender(socket, { "device" }, { 1 }, idFactory = { "t" })
    ...
    @Test fun commitFrameCarriesFrozenIdentityLengthAndDigestFields() {
        val socket = FakeSocket()
        val sender = ContentTransferSender(socket, { "d1" }, { 1 }, idFactory = { "t1" })
    ...
    @Test fun queuedControlInterleavesAfterAtMostOneBulkChunkAndCapacityIsBounded() {
        val socket = FakeSocket(); val sender = ContentTransferSender(socket, { "d" }, { 1 }, idFactory = sequenceOf("c", "t").iterator()::next)
