...
3. Missing v1 values are derived from the bundled manifest: `bin/node_repl`, `bin/node`, `lib/node_modules`, and `share/playwright`. After the native launcher executes `bin/node`, the host independently derives the runtime root and co-installed Browser package path from `process.execPath`; no Browser trust selector is exported into the process.
