...
if [[ -n "${WAYLAND_DISPLAY:-}" && -S "$XDG_RUNTIME_DIR/$WAYLAND_DISPLAY" ]]; then
  ...
  if [[ -n "$DISPLAY" && -f "$server_auth" ]]; then
    ...
    export XAUTHORITY="${XDG_RUNTIME_DIR:-/tmp}/sky-cua-targeted-screenshot.Xauthority"
    rm -f "$XAUTHORITY"
    touch "$XAUTHORITY"
...
if [[ "${XDG_SESSION_TYPE:-}" != "wayland" && "${XDG_SESSION_TYPE:-}" != "x11" ]]; then
  printf 'targeted screenshot profile requires a real Wayland or X11 session; WAYLAND_DISPLAY=%s DISPLAY=%s XAUTHORITY=%s\n' "${WAYLAND_DISPLAY:-}" "${DISPLAY:-}" "${XAUTHORITY:-}" >&2
  exit 67
fi

exec python scripts/live_targeted_screenshot_smoke.py
