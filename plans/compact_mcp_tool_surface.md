...
- [ ] Add authoritative machine fixtures for the legacy and compact tool registries, compact branch contracts, minimal valid and invalid compact call fixtures, plus a generated or validated Markdown ledger.
- [ ] Add immutable MCP process/session configuration and enforce that advertised tool names are exactly the callable tool names for the active profile.
- [ ] Add the host schema and approval compatibility smoke before relying on grouped compact schemas.
- [ ] Implement compact read-only tools with stable compact response envelopes and profile-aware prose.
- [ ] Implement compact action tools split by approval class, coordinate contract, and schema complexity.
- [ ] Persist profile selection through every installer and host-launch path while preserving legacy for existing installs.
