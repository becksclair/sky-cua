...
Shipped. Last verified: 2026-06-13 local unit checks, build validation, KDE
Wayland live smokes, Windows cross-target compile checks, and native DevBox
Windows unit tests. Linux VM matrix and an interactive-session Windows live
smoke gate are tracked in Verification.
...
Most recent accepted live artifacts:

- App-server smoke:
  `artifacts/codex-e2e/app-server-smoke/20260613T134541Z`
...
- Multi-output VM proof depends on the selected guest exposing more than one
  ...
  secondary-display assertion when only one output exists.
