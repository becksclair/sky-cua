...
- [ ] (2026-07-20 07:34Z, partial) WP-02: Migrated the complete first-party `cua_node` source/runtime contract and locked assets from the preserved Codex Desktop input into `runtime/cua-node/**`, committed as `530deb5`. Source verification passes 191 tests with 7 intentional production skips. The offline assembler rebuilds first-party outputs under one publication lock, binds the exact 323,846,851-byte/3,579-file migration seed (`b6677535a94b231fcf681ee30d2d1d42ef8028095d45e4040c7eefacda6c32d2`), inventories every shipped package root, emits package/data/native license evidence and SBOM associations, audits native binaries, rejects wrong-platform packages, and marks dirty-source output release-ineligible. Clean component `1027e592079e19472fdf824b8c072700c9ddc72ca16caf6bd9fb12213986f93a` (3,798 files, 324,854,601 bytes) passed independent check, exact installed transcript, 12-cell REPL media, full offline Canvas/Sharp/PDF.js/Tesseract/pixelmatch/system-Chrome Playwright, two-cycle web lifecycle, and packaged Browser trust negatives. Its benchmark exposed a checkout-lock default in the harness; after committing that repair and this plan, one final clean rebuild must replace this otherwise-valid component before WP-02 closes.
...
- [ ] (2026-07-20 07:34Z, partial) WP-07: Ported exact runtime/package/native/data-asset locks, notices, provenance, CycloneDX/SPDX, and recorded the hash-bound current Codex migration-input baseline (`65c69a3`, runtime SHA-256 `eed2bb02daf9ed79add6e96b2b759897e04bde4b393d3049b7fdde4cb93e6b95`). Source runtime verification passes 191 tests with 7 production skips. The clean installed component passes exact MCP transcripts, persistence/reset/timeout/cancel/meta, native/local-file/buffer/URL/image operations, OCR/PDF/Sharp/Canvas/pixelmatch, WebP/PNG, standalone Playwright, web lifecycle, and all three packaged Browser trust negatives before connection. The first installed benchmark failed closed because its CLI enforced checkout lock paths; the default now resolves the candidate generation's own manifest-bound `share/locks`, with an exact regression. `@heliasar/sky-cua` exposes additive v1 `activate_window` context/capability and a packaged full-action runner; its typecheck/build and 47 tests pass, with the window-action contract committed as `83dd279`. Remaining work is the rebuilt candidate benchmark, installed Browser/Computer live-host runs, and direct-no-Node-hop topology proof.
